    ...
    public function toMail($notifiable)
    {
        if ($this->thread->notificationType == 1) { //inbox notification
            return (new MailMessage)
                        ->subject('Ny meddelelse!')
                        ->line(auth()->user()->userName.' har sendt dig en besked.')                        
        ...
        elseif ($this->thread->notificationType == 5) { //new portal registration
            $email = new MailMessage();
            $email->line('Thanks for joining ' . Portal::find($this->thread->portal_id)->portalType) 
            ->action('Login', url('/'))
            ->subject('Welcome to ' . Portal::find($this->thread->portal_id)->portalType) 
            ->line('Vi håber du er glad for vores side og modtager gerne både ris og ros på hello@datingportalen.com.');           
            return  $email;
        }
        elseif ($this->thread->notificationType == 6) { //newly  registration
            $email = new MailMessage();
            $email->line('Your registration and plan subscribed successfully')
            ->action('Login', url('/'))
            ->subject('Welcome to ' . Portal::find($this->thread->portal_id)->portalType)
            ->line('Vi håber du er glad for vores side og modtager gerne både ris og ros på hello@datingportalen.com.');            
            return  $email;
